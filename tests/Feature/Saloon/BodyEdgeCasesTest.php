...
use Milzer\HttpLogger\Tests\Support\CustomBodyRequest;
use Milzer\HttpLogger\Tests\Support\GetBookingRequest;
use Milzer\HttpLogger\Tests\Support\MultipartFilesRequest;
...
it('omits resource bodies larger than the limit', function (): void {
    ...
    expect(testLog()->record(0)->get('http.body'))->toBe('[body omitted: stream larger than the 5 B limit]');
});

it('omits streams of unknown size once they exceed the limit', function (): void {
    useOptions(new LoggingOptions(maxParseBytes: 5, throwOnError: true));
...
it('omits string bodies larger than the limit', function (): void {
    ...
    expect(testLog()->record(0)->get('http.body'))->toStartWith('[body omitted: ')
        ->toEndWith('larger than the 10 B limit, application/json]');
});
